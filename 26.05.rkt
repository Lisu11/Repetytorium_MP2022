...
; (5 + 7) * 9 [<>]
; 5 + 7 [λ z. z * 9 | <>]
; 5 [λ x. opE + x 7 | λ z. opE * z 9 | <>]
; 7 [λ z. 5 + z | λ z. opE * z 9 | <>]
; 12 [λ z. opE * z 9 | <>]
; 9 [λ x . 12 * x | <>]
; 108 [<>]
; 108
...
(define mtD : StackD empty)
(define (pushD [e : Exp] [s : StackD]) : StackD
  (cons e s))
(define (popD [s : StackD]) : (Exp * StackD)
  (type-case StackD s
    [empty
     (error 'popD "empty stack")]
    [(cons v s)
     (pair v s)]))

(define (decompile [c : Code]) : Exp
  (local ((define (aux [c : Code] [s : StackD])
    (type-case Code c
      [empty
       (fst (popD s))]
      [(cons i c)
